...
the feasibility of a program path. One can argue that it can also be refactored out
...
higher-level programming language\sidenote[][-2cm]{An abstract domain consists of data
...
\end{minted}
    ...
    \ccode{i}.  Therefore, the result of condition \ccode{b < 0} might be
...
successful verification of \llvm programs. Especially, I want to focus on how
to generalize program abstractions. Depending on the target of abstraction, it
requires various data representations and different analyses. However, I want
to minimize the number of various abstraction types. Hence in the

...
terminal instruction is for example branching or return instruction.}
...
\end{minipage}
    ...
    (\textbf{left}), after 5 iterations (\textbf{middle}), at the reached fixpoint
    (\textbf{right}).}
    \label{fig:signcomp}%
    \end{adjustwidth}
\end{figure}%
...
well-defined domain is required to have an image for each concrete value, and
 ...
 \begin{definition}
 For a concrete lattice $(C, \subseteq)$ and abstract lattice $(A,
...
sign domain \domain{sign} was not able to determine any interesting invariants,
...
\resizebox{\textwidth}{!}{
    ...
    \caption{Lattice of constant propagation abstract domain \domain{cp}.}
    \label{fig:cp}%
\end{marginfigure}%
...
representation of abstract values.\sidenote[][-3cm]{\footnotesize\textit{Unique
representation is highly demanded in the program verification because it allows
fast equality comparisons of abstract states.}} The octagon domain was further
...
\caption{Counterexample guided abstraction refinement loop.}
\label{fig:cegar}
\end{figure}

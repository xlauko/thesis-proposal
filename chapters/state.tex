...
\begin{marginfigure}%
    ...
    \draw [thin] (t) -- (b);
    \end{tikzpicture}
    \caption{\domain{U} keeps only information about definedness of variables.}
    \label{fig:unitd}%
\end{marginfigure}%
...
\end{tikzpicture}
    ...
    \caption{Result of interval abstraction for variable \llvmint{|\var{\%x}|} after fixpoint iteration.}
    \label{fig:interval}
\end{marginfigure}
...
$\mathcal{M} = \mathcal{E} \times \mathcal{H}$, for better illustration see
example in \autoref{fig:memory}.

\begin{marginfigure}
    \add{ TODO memory example }
...
For more precise updates, pointer abstraction leverage \emph{separation logic}.
It allows performing local reasoning about memory (i.e., consider only a part
of memory). In separation logic, the memory relations are described by
points-to predicates and separating conjunctions on heaps~\cite{Reynolds2002}.

However, just separation logic is too expressive to be tractable as an abstract
domain because it is not efficient manipulation. In static analysis, separation
logic is usually used in combination with shape analysis. Shape graphs are used
to generalize abstract data structures (e.g., lists or trees). The shape graph
can summarize inductive data structures into simple
shapes~\cite{Distefano2006}. Similarly to scalar analysis, shape analysis can
be refined by widening or by keeping shape relations~\cite{Chang2008}.  Reduced
products to shape analysis was introduced in~\cite{Toubhans2013}.
...
semantic reasoning to \textsc{smt} solvers and concentrates on the design of
...
set of pa/-ra/-me/-trized predicates which are further gradually refined via \textsc{smt}.
Moreover, the \textsc{smt} representation enables a straightforward combination with
...
The simplest and heavily used data structure \emph{memory array} gets a lot of
...
requires being able to reason about relationships between array size and array
...
numerical abstract domains by which tha abstraction represents the internal
...
is more precise to treat them separately. In C language, strings are
...
the properties of bit-precise arithmetic like overflows in the abstract domains.
This problem is solved in recent abstract domains that leverage bit-vector
logic to reproduce processor semantics~\cite{Sharma2013, Sharma2017}. Similar
domains were also designed for floating-point arithmetic~\cite{Putot2004}.  The
real-world program analysis also requires to reflect memory model of the
program under test. To describe memory model properties, analysis usually
leverage dedicated predicate abstraction~\cite{Dan2013}.
...
\add{example of bitvector abstraction which is not recogniazable by infinite domain}
...
More domain-specific abstractions are used to analyze database querying languages~\cite{Halder2012}; system components like filters~\cite{Feret2004} or synchronous clocks~\cite{Feret2005}.
...
abstract unions. The usual usecase of abstract dijunctions (unions) is to merge
multiple paths of the program. However, many abstract domains are not closed
under union operation.  \emph{Disjunctive completion methods} address this
problem \cite{File1999}. The complementation introduces to abstraction all
...
built on top of \llvm.  It is closely related to our course, mainly because it
...
\textsc{crab} domains reason only about linear arithemtic; it lacks any pointer
...
In model checking, abstraction is used to reduce the size of model state
space~\cite{Pelnek2006}.  Instead of performing exploration of concrete state
space, algorithms group a set of concrete states together according to some
abstraction to explore a reduced state space. Model checkers also use
...
reachable in the original concrete model; if not so, we are talking about the
spurious error.
...
theory explication used in \textsc{smt} solvers. Eager strategy in \textsc{smt} solvers
abstract the formula to its propositional skeleton -- to compensate the loss of
information \textsc{smt} solver adds tautologies to the formula before abstraction,

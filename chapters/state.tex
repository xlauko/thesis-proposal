...
well-defined domain is required to have an image for each concrete value, and
 ...
 $\alpha : C \rightarrow A$, $\gamma : A \rightarrow C$ are monotonic functions
 such that:
...
\add{image}
\caption{Store-based model of heap graphs.}
...
\begin{definition}
    ...
    \begin{itemize}
        \itemsep0em
        \item $\rho: \mathcal{R} \rightarrow \mathcal{C} \cup
            \mathcal{A}_{\textit{addr}}$ registers may contain abstract
            addresses,
        \item $\epsilon : \mathcal{V} \rightarrow \mathcal{C}_{\textit{addr}}
            \cup \mathcal{A}_{\textit{addr}}$ environment allows to assign
            objects symbolic addresses, and
        \item $\mu: \mathcal{C}_{\textit{addr}} \cup
            \mathcal{A}_{\textit{addr}} \rightarrow \mathcal{C} \cup
...
\add{example of bitvector abstraction which is not recognizable by infinite domain}
\caption{Bitvector}
\label{fig:bitvector}
\end{marginfigure}
